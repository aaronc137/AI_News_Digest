...
### 日报语言契约（2026-09-22 用户确认）

风格参照：[以研究驱动的产品探索，保留 AI 时代领先的智能和组织能力](https://bytedance.larkoffice.com/docx/CCFFdDQakodQWDx5Hu6cfnhEnyd)（已读修订版 27）。只借鉴表达与论证方法，不将参考文档的事实、预测和组织主张自动纳入日报，也不照搬其长文篇幅与正式程度。

- **判断直接，依据具体**：趋势首句说清本期判断，随后用公司、产品、动作和证据解释为什么成立，再落到受影响的人与决策。不用“值得注意的是”“不难发现”“这标志着”起手，不强制每段套同一结构。
- **因果写全，不靠概念跳跃**：说明谁做了什么、改变了哪个条件、为什么影响工作或选择。用“保留任务状态，让中断的工作可以继续”解释机制，不以“赋能长程闭环”代替解释；术语确有必要才保留。
- **具体且自然，不用口号充当钩子**：标题保留公司或产品名及本期变化；通俗不等于拟人化、喊话或故作俏皮。“AI 接下一步”“模型强不强不重要了”等泛判断不能替代新闻。“胜负手、复利、闭环、重塑格局、窗口关闭”须有对应机制与证据，不能因参考文档使用就反复套用。
- **有判断，不制造确定性**：区分发布事实、作者观点与编辑推断；用“我们判断”“公开材料尚未确认”等简短限定，仅在需要处使用。产品功能不等于实际效果，融资不等于盈利，收购不证明训练闭环已建立；不能为增强语气删去开放范围与证据边界。
- **一句一事，段落有推进**：优先明确主语、动词和短句，拆开多层定语与抽象名词串。各段分别提供增量，不在标题、首句、结尾重复同一判断；加粗少数关键结论和数字，不整段加粗。
- **减少机械对仗与排比**：不要连续使用“不是……而是……”“从……走向……”“真正的……在于……”或每段三点式总结；确有比较才写对照。删去不能增加事实、机制或影响的转折、感叹与升华句。
- **正文不展示编辑过程**：不写“Coze 日报整理的”“这条值得捞回来”“暂不写成已确认事实”等工作记录。必要的不确定性直接附着在主张上，例如“据该媒体报道，官方尚未公布”；抓取失败、去重和选稿过程单独留在研究记录或简短覆盖说明中。
- **按产物调节密度，不改变格式**：详细版可以展开因果；卡片用完整的亮点概括，不截取原文首字加省略号；结构化表格只沉淀新闻事实与有归属的观点，不另加批判性判断；趋势图仅留具体命题和必要边界。沿用既有表格、信号灯、链接与卡片模板。

语言终审必须逐条检查：①没有公司名也能每天套用吗？②判断能否逐步追溯到本期证据？③是否用抽象词掩盖了动作或因果？④是否反复使用同一句式或重复结论？⑤删掉修辞后还剩什么信息？发现问题改写对应句段，不机械替换同义词，也不以“像人写的”为由放松事实审核。
...
- **语言契约是否通过**：按“日报语言契约（2026-09-22）”检查具体判断、因果与证据、抽象词堆叠、重复句式和空泛修辞；事实边界不能在润色中消失，正文不得夹带编辑过程。
